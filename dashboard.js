...
// ============================================
// ID GENERATION: DATE + UNIQUE 6-DIGIT NUMBER (FIXED)
// ============================================
async function generateUniqueId() {
    ...
    // Convert date prefix to number range (e.g., 20260103000001 to 20260103999999)
    const minId = BigInt(datePrefix + "000001");
    const maxId = BigInt(datePrefix + "999999");

    // Fetch all products created today (within the date range)
    const { data, error } = await supabaseClient
        .from("products")
        .select("id")
        .gte("id", minId.toString())  // Greater than or equal to min
        .lte("id", maxId.toString()); // Less than or equal to max

    if (error) {
        console.error("Error fetching today's products:", error);
        // Continue anyway - just start from 1 if query fails
    ...
    const existingNumbers = (data || []).map(p => {
        ...
        if (idStr.length === 14) {
            const sixDigit = idStr.substring(8); // Last 6 digits
    ...
    // Pad to 6 digits and combine with date
...
form.addEventListener("submit", async e => {
    ...
    const product = {
        id: parseInt(finalId), // Convert to number since your column is bigint
